...
public class HttpServiceTest {
    ...
    public void httpException() throws IOException {
        ...
        Request<String, BhpBlockCount> request = new Request<>(
                "getblockcount",
                Collections.emptyList(),
                mockedHttpService,
                BhpBlockCount.class);
        try {
            mockedHttpService.send(request, BhpBlockCount.class);
    ...
    public void testAsyncWithExternalExecutor() throws ExecutionException, InterruptedException {
        ...
        Request<String, BhpBlockCount> request1 = new Request<>(
                "getblockcount",
                Collections.emptyList(),
                mockedHttpService1,
                BhpBlockCount.class);

        mockedHttpService1.sendAsync(request1, BhpBlockCount.class);
        Assert.assertThat(executor.isCalled(), is(true));

        HttpService mockedHttpService2 = new HttpService(executor, false);

        Request<String, BhpBlockCount> request2 = new Request<>(
                "getblockcount",
                Collections.emptyList(),
                mockedHttpService2,
                BhpBlockCount.class);

        CompletableFuture<BhpBlockCount> result = mockedHttpService2.sendAsync(request2, BhpBlockCount.class);
        Assert.assertThat(executor.isCalled(), is(true));
    }
