...
import static io.bhpw3j.utils.Assertions.verifyPrecondition;
...
public class Sign {
    ...
    public static BigInteger recoverFromSignature(int recId, ECDSASignature sig, byte[] message) {
        ...
        //   1.1 Let x = r + jn
        BigInteger n = BHPConstants.CURVE.getN();  // Curve order.
        ...
        ECPoint q = ECAlgorithms.sumOfTwoMultiplies(BHPConstants.CURVE.getG(), eInvrInv, R, srInv);
    ...
    private static ECPoint decompressKey(BigInteger xBN, boolean yBit) {
        ...
        byte[] compEnc = x9.integerToBytes(xBN, 1 + x9.getByteLength(BHPConstants.CURVE.getCurve()));
        compEnc[0] = (byte) (yBit ? 0x03 : 0x02);
        return BHPConstants.CURVE.getCurve().decodePoint(compEnc);
    }

    /**
     * Given an arbitrary piece of text and an BHP message signature encoded in bytes,
    ...
    public static ECPoint publicPointFromPrivateKey(BigInteger privKey) {
         ...
         */
        if (privKey.bitLength() > BHPConstants.CURVE.getN().bitLength()) {
            privKey = privKey.mod(BHPConstants.CURVE.getN());
        }
        return new FixedPointCombMultiplier().multiply(BHPConstants.CURVE.getG(), privKey).normalize();
    }
