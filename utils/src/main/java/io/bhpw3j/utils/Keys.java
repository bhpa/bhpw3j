...
import io.bhpw3j.contract.ScriptBuilder;
import io.bhpw3j.contract.ScriptHash;
import io.bhpw3j.crypto.Base58;
import io.bhpw3j.crypto.Hash;
import io.bhpw3j.crypto.exceptions.AddressFormatException;
...
import static io.bhpw3j.constants.BHPConstants.MAX_PUBLIC_KEYS_PER_MULTISIG_ACCOUNT;
...
public class Keys {
     ...
     * @deprecated Use {@link io.bhpw3j.contract.ScriptHash#fromPublicKey(byte[])} instead.
     */
    @Deprecated
    ...
    public static byte[] publicKeyIntegerToByteArray(BigInteger publicKey) {
        return Numeric.toBytesPadded(publicKey, BHPConstants.PUBLIC_KEY_SIZE);
    }

    public static byte[] privateKeyIntegerToByteArray(BigInteger privateKey) {
        return Numeric.toBytesPadded(privateKey, BHPConstants.PRIVATE_KEY_SIZE);
    }
    ...
    public static byte[] getPublicKeyEncoded(byte[] publicKeyNotEncoded) {
        ...
        if (publicKeyArray[64] % 2 == 1) {
            return concatenate(new byte[]{0x03}, Arrays.copyOfRange(publicKeyNotEncoded, 1, BHPConstants.PUBLIC_KEY_SIZE));
        } else {
            return concatenate(new byte[]{0x02}, Arrays.copyOfRange(publicKeyNotEncoded, 1, BHPConstants.PUBLIC_KEY_SIZE));
        }
    }
    ...
    public static String toAddress(byte[] scriptHash) {
        ...
        data[0] = BHPConstants.COIN_VERSION;
    ...
    public static boolean isValidAddress(String address) {
        ...
        if (data[0] != BHPConstants.COIN_VERSION) return false;
     ...
     * @deprecated Use {@link io.bhpw3j.contract.ScriptHash#fromAddress(String)} instead.
     */
    @Deprecated
    public static byte[] toScriptHash(String address) {
        if (!isValidAddress(address))
            throw new IllegalArgumentException("Not a valid BHP address.");
