...
public class ScriptHash implements Comparable<ScriptHash> {
    ...
    private void checkAndThrowHashLength(byte[] scriptHash) {
        if (scriptHash.length != BHPConstants.SCRIPTHASH_LENGHT_BYTES &&
                scriptHash.length != BHPConstants.ASSET_ID_LENGHT_BYTES) {

            throw new IllegalArgumentException("Script hash must be either " +
                    BHPConstants.SCRIPTHASH_LENGHT_BYTES + " or " + BHPConstants.ASSET_ID_LENGHT_BYTES +
