...
import io.bhpw3j.transaction.ContractTransaction;
...
public class AssetTransfer {
    ...
    public static class Builder {
        ...
        public AssetTransfer build() {
            if (bhpw3J == null) throw new IllegalStateException("Bhpw3j not set");
        ...
        private void handleTransferFromContract(Map<String, BigDecimal> requiredAssets) {
            ...
            if (this.utxos.isEmpty()) {
                try {
                    contractAcct.updateAssetBalances(bhpw3J);
                } catch (Exception e) {
            ...
            BhpGetContractState contractState;
            try {
                contractState = bhpw3J.getContractState(fromContractScriptHash.toString()).send();
        ...
        private Map<String, BigDecimal> calculateRequiredAssetsForIntents(
            ...
            calculateServiceFee(assets, outputs);

            return assets;
        }

        private void calculateServiceFee(Map<String, BigDecimal> assets, List<RawTransactionOutput> outputs) {
            BigDecimal value = assets.get(BhpAsset.HASH_ID);
            if (!outputs.isEmpty()) {
                if (null != value && value.compareTo(BigDecimal.ZERO) > 0) {
                    int size = outputs.size() * 60;
                    int base = Math.min((size / 512) + ((size % 512) == 0 ? 0 : 1), 5);
                    BigDecimal serviceFee = BigDecimal.valueOf(base).multiply(BigDecimal.valueOf(0.0001));
                    BigDecimal after = value.add(serviceFee);
                    assets.put(BhpAsset.HASH_ID, after);
                }
            }
        }
