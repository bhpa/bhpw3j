...
import io.bhpw3j.protocol.core.methods.response.InvocationResult;
...
public class ContractInvocation {
    ...
    public InvocationResult testInvoke() throws IOException, ErrorResponseException {
        ...
        if (function != null) {
            if (params.isEmpty()) {
                response = bhpw3J.invokeFunction(scriptHash.toString(), function).send();
            } else {
                response = bhpw3J.invokeFunction(scriptHash.toString(), function, params).send();
            }
        } else {
            response = bhpw3J.invoke(scriptHash.toString(), params).send();
        }
        response.throwOnError();
